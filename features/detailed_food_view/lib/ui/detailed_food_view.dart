import 'package:core/core.dart';
import 'package:core_ui/core_ui.dart';
import 'package:core_ui/widgets/app_button.dart';
import 'package:core_ui/widgets/app_image.dart';
import 'package:core_ui/widgets/number_increasing.dart';
import 'package:domain/domain.dart';
import 'package:flutter/material.dart';
import 'package:core_ui/widgets/ingredient_item.dart';

class DetailedFoodView extends StatelessWidget {
  final FoodModel foodModel;

  const DetailedFoodView({
    super.key,
    required this.foodModel,
  });

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      appBar: AppBar(
        title: Text(StringConstants.appBarTitle,
            style: Theme.of(context).appBarTheme.titleTextStyle),
        backgroundColor: Theme.of(context).scaffoldBackgroundColor,
        elevation: AppDimens.elevetion_0,
        centerTitle: true,
        leading: const BackButton(color: AppColors.green),
      ),
      body: SingleChildScrollView(
          scrollDirection: Axis.vertical,
          child: Container(
              child: Column(
            children: [
              Align(
                child: Container(
                  margin: const EdgeInsets.only(top: AppDimens.P_45),
                  width: MediaQuery.sizeOf(context).width / 1.5,
                  height: MediaQuery.sizeOf(context).height / 3.6,
                  decoration: BoxDecoration(
                    borderRadius: const BorderRadius.all(
                      Radius.circular(AppDimens.P_20),
                    ),
                    boxShadow: const <BoxShadow>[
                      BoxShadow(
                        color: AppColors.shadowBlack,
                        blurRadius: AppDimens.blurRadiusShadow_20,
                        offset: AppDimens.boxShadowOffset,
                      ),
                    ],
                    color: Theme.of(context).cardColor,
                  ),
                  child: AppImage(
                    imageURL: foodModel.url,
                  ),
                ),
              ),
              const SizedBox(
                height: AppDimens.verticalSpacing_40,
              ),
              NumberIncreasing(),
              const SizedBox(
                height: AppDimens.verticalSpacing_40,
              ),
              Text(
                foodModel.name,
                style: Theme.of(context).textTheme.headlineSmall,
              ),
              const SizedBox(
                height: AppDimens.verticalSpacing_20,
              ),
              Text(
                '${foodModel.price}\$',
                style: Theme.of(context).textTheme.titleLarge,
              ),
              const SizedBox(
                height: AppDimens.verticalSpacing_20,
              ),
              Container(
                padding: const EdgeInsets.only(
                  right: AppDimens.P_35,
                  left: AppDimens.P_35,
                ),
                child: Text(
                  foodModel.description,
                  textAlign: TextAlign.justify,
                  style: AppFonts.normal_14,
                ),
              ),
              const SizedBox(
                height: AppDimens.verticalSpacing_20,
              ),
              const Text(
                StringConstants.ingredientsTitle,
                style: AppFonts.bold_14,
              ),
              Container(
                alignment: Alignment.center,
                child: Wrap(
                  children: List.generate(
                    foodModel.ingredients.length,
                    (index) => Container(
                      margin: const EdgeInsets.all(AppDimens.P_10),
                      width: MediaQuery.of(context).size.width / 3,
                      child: IngredientItem(
                        title: foodModel.ingredients[index],
                      ),
                    ),
                  ),
                ),
              ),
              const SizedBox(
                height: AppDimens.verticalSpacing_80,
              ),
            ],
          ))),
      bottomSheet: AppButton(
        onTap: () {}, //TODO add to cart logic
        width: MediaQuery.of(context).size.width / 7,
        margin: const EdgeInsets.only(
          bottom: AppDimens.P_20,
          right: AppDimens.P_15,
          left: AppDimens.P_15,
        ),
        padding: AppDimens.P_20,
        title: StringConstants.addToCartString,
        textStyle: AppFonts.white_bold_14,
        boxDecoration: const BoxDecoration(
          borderRadius: BorderRadius.all(Radius.circular(AppDimens.P_20)),
          gradient: AppColors.primaryGradient,
        ),
      ),
    );
  }
}
